...
public class StrategoController {
	...
	public void nouveauTour() {
		if (joueur1.estPret()) {
			...
			switch (reponse) {
			case "deplacer": deplacerPion(); break;
			default: joueur1.acheterUnite(model.Unite.convertirStrUnite(console.acheterPion()));
			}
	...
	public void deplacerPion() {
		...
		plateauConsole.placerPion(xArrivee, yArrivee, plateau.pionsEnJeu.get(plateau.map[xDepart][yDepart]).getUnite().getGrade());
		plateauConsole.placerPion(xDepart, yDepart, 0);
	}
}
